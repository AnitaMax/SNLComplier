    ...
    public AnalyseResult parsing(List<Token> tokens, ProductionElement startEle) {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (analysingStack.size() <= 0) {
                result.getLogs().add(new AnalyseLog(token, analysingStack, "栈空但是还有输入！"));
            ...
            if (stackTop.isEnd()) {
                if (stackTop.accept(token)) {
                    result.getLogs().add(new AnalyseLog(token, analysingStack, "匹配"));
                    analysingStack.pop();
                } else {
                    result.getLogs().add(new AnalyseLog(token, analysingStack, "栈顶不匹配"));
                    result.setSuccess(false);
            ...
            //栈顶是非终极符，根据输入的终极符展开
                    ...
                    if (producton.getPredict() == null || producton.getPredict().isEmpty()) {
                        ...
                        for (var pred : producton.getPredict()) {
                            if (!success && pred.accept(token)) {
                                success = true;
                                result.getLogs().add(new AnalyseLog(token, analysingStack, "替换 " + producton.toString()));


                                //树形结构
                                ArrayList<Node> leafNodes = result.getTree().getLeafNode();
                                String l_name = producton.getLeft().getContent();
                                for (Node rootNode : leafNodes) {
                ...
                if (!success) {
                    result.getLogs().add(new AnalyseLog(token, analysingStack, "没有相应的产生式"));
                    result.setSuccess(false);
        ...
        if (analysingStack.isEmpty()) {
            result.getLogs().add(new AnalyseLog(null, analysingStack, "成功！"));
